...
object HomeworkSpec {
  ...
  @JsonCodec final case class GameStats(hTeam: TeamBoxScore,
                                        vTeam: TeamBoxScore)

  @JsonCodec final case class PrevMatchup(gameDate: LocalDate, gameId: String)

  val format = DateTimeFormatter.BASIC_ISO_DATE
  implicit val decodeLocalDate: Decoder[LocalDate] = Decoder.decodeString.emap {
    date =>
      Either
        .catchNonFatal(LocalDate.parse(date, format))
        .leftMap(
          err => "LocalDate " + date + " parsing error: " + err.getMessage
        )
  }

  @JsonCodec final case class BoxScore(basicGameData: Game,
  ...
  @JsonCodec final case class TeamStats(linescore: List[JustScore],
                                        loss: String,
                                        score: String,
                                        teamId: String,
                                        triCode: String)
  ...
  @JsonCodec final case class Arena(city: String,
                                    country: String,
                                    isDomestic: Boolean,
                                    name: String,
                                    stateAbbr: String)

  @JsonCodec final case class Game(arena: Arena,
                                   attendance: String,
  ...
  private def fetchGameInfo(date: LocalDate,
                            gameId: String): Either[circe.Error, BoxScore] = {
    val jsonString = Try {
